...
function getAccent(symbol: string, index: number): Accent {
  ...
  if (normalized === "fe" || normalized === "iron") {
    return {
      ...
      chip: "bg-amber-700/10 text-amber-700 dark:text-amber-500",
      glow: "from-amber-700/20 via-amber-700/5 to-transparent",
    };
  }
...
export const Route = createFileRoute("/crops_/$slug/nutrients")({
  loader: async ({ params }) => {
    try {
      const { data: cropData, error: cropError } = await withTimeout(
        supabase.from("crops").select("*").eq("slug", params.slug).maybeSingle(),
      );

      if (cropError) throw cropError;

      if (cropData) {
        const { data: defData, error: defError } = await withTimeout(
          supabase
...
function CropNutrients() {
  ...
  const localizedDeficiencies = deficiencies.map((d: any, i: number) => {
    const name =
      typeof d.name === "object" && d.name ? d.name[language] || d.name["en"] || "" : d.name;
    const symptoms =
      typeof d.symptoms === "object" && d.symptoms
        ? d.symptoms[language] || d.symptoms["en"] || ""
        : d.symptoms;
    const affect =
      typeof d.affect === "object" && d.affect
    ...
    const product =
      typeof d.product === "object" && d.product
        ...
        : d.product;
    const soilDrip =
      typeof d.soil_drip === "object" && d.soil_drip
    ...
    const benefit =
      typeof d.benefit === "object" && d.benefit
    ...
    const img = "image_url" in d && d.image_url ? d.image_url : d.img || cropImg;
    ...
    <>
      ...
      <section className="border-b border-border bg-background/95 supports-[backdrop-filter]:bg-background/80 backdrop-blur-md sticky top-20 z-30 shadow-sm">
        <div className="max-w-6xl mx-auto px-6 py-4 flex flex-wrap items-center gap-3">
          ...
          {localizedDeficiencies.map((d: any, i: number) => (
            <a
              key={d.name}
              href={`#${slugify(d.name)}`}
        ...
        <div className="max-w-6xl mx-auto px-6 relative">
          ...
          <div className="max-w-3xl mb-14">
            ...
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight">
              Common deficiencies in <span className="text-primary">{cropName.toLowerCase()}</span>{" "}
              &amp; how to fix them
            </h2>
          ...
          <div className="space-y-16">
            {localizedDeficiencies.map((d: any, i: number) => (
              <DeficiencyCard
                ...
                d={d}
                index={i}
                totalCount={localizedDeficiencies.length}
              />
            ))}
          </div>
            ...
            <div className="relative grid md:grid-cols-[1.6fr_1fr] gap-8 items-center">
              <div>
                ...
                <p className="mt-3 text-white/80 max-w-xl">
                  Send a photo of your field to our agronomist — get a personalised spray schedule
                  for your {cropName.toLowerCase()} crop within 24 hours.
                </p>
              </div>
...
function DeficiencyCard({ d, index, totalCount }: { d: any; index: number; totalCount: number }) {
    ...
    >
      ...
      >
        ...
        <div className="p-8 md:p-12">
          <h3 className="text-3xl md:text-4xl font-bold tracking-tight leading-tight">{d.name}</h3>
...
function InfoTile({
  ...
  return (
    <div className="rounded-2xl border border-border bg-background/40 p-5">
      <div
        className={`inline-flex items-center gap-2 rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${toneCls}`}
      >
        {icon} {label}
      </div>
